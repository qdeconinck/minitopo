...
class MpExperience:
	...
	def changeMetric(self):
		...
		if int(metric) >= 0:
			self.mpTopo.notNSCommand("echo " + metric + " | sudo tee /sys/module/mptcp_sched_metric/parameters/metric")

	def changeOpenBup(self):
		openBup = self.xpParam.getParam(MpParamXp.OPENBUP)
		if int(openBup) >= 0:
			self.mpTopo.commandTo(self.mpConfig.client, "echo" + openBup + " | sudo tee /sys/module/mptcp_fullmesh/parameters/open_bup")
			self.mpTopo.commandTo(self.mpConfig.server, "echo" + openBup + " | sudo tee /sys/module/mptcp_fullmesh/parameters/open_bup")
